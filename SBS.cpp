...
int writepr()
{
    fp.open("SMBS.txt",ios::out|ios::app);
    pr.createpr();
...
int disp()
{
    ...
    fp.open("SMBS.txt",ios::in|ios::out);
...
int deletepr()
{
    ...
    fp.open("SMBS.txt",ios::in|ios::out);
    fstream fp2;
    fp2.open("Temporary.txt",ios::out);
    fp.seekg(0,ios::beg);
    while(fp.read((char*)&pr,sizeof(product)))
    {
        if(pr.retprno()!=num)
...
int admin()
{
    ...
    cout<<"2.Modify/Update a product"<<endl;
...
int main()
{
    ...
    do
    {
        cout<<"\n\nWelcome to Super Market Billing System Application"<<endl;
        cout<<"Main Menu:"<<endl;
